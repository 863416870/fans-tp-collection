<template>
  <div
    class="out"
    :class="{ 'l-liang-out': isLLiang,'l-you-out':isLYou,'l-qing-out':isLQing,'l-zhong-out': isLZhong, 'l-zhongdu-out':isLZhongDu }"
    @mouseenter="onMouseOver"
    @mouseleave="onMouseout"
  >
    <div
      class="in"
      :class="{ 'l-liang-in': isLLiang,'l-you-in':isLYou,'l-qing-in':isLQing,'l-zhong-in': isLZhong, 'l-zhongdu-in':isLZhongDu }"
    ></div>
    <div class="point-info" v-if="seen">
      <div class="point-item" v-show="showStatus.deviceName">监测点：{{deviceName}}</div>
      <div class="point-item" v-show="showStatus.zoneName">位置：{{zoneName}}</div>
      <div class="point-item" v-show="showStatus.pollutant">首要污染物：{{pollutant}}</div>
    </div>
  </div>
</template>
