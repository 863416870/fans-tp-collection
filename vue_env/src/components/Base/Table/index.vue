<template>
  <div class="zzd-table">
    ...
    >
      ...
      <el-table-column v-if="index" :type="index" :index="currentIndex" width="55" />
      <el-table-column
  ...
  methods: {
    ...
    rowClick(row, column, event) {
      ...
      if (!this.oldKey.includes(row[this.selectFieldUnique])) {
        ...
        this.oldKey = this.oldKey.filter(
          item => item !== row[this.selectFieldUnique]
        );
        const data = this.oldVal.filter(
          item => item[this.selectFieldUnique] !== row[this.selectFieldUnique]
        );
        this.handleSelectionChange(data);
        this.toggleSelection(
      ...
      if (
        this.currentOldRow &&
        this.currentOldRow[this.selectFieldUnique] ===
          row[this.selectFieldUnique]
      ) {
        // 取消单选选中
    ...
    currentChange(page) {
      ...
      this.selectedTableData.forEach(item => {
        for (let i = 0; i < this.currentData.length; i++) {
          if (
            this.currentData[i][this.selectFieldUnique] ===
            item[this.selectFieldUnique]
          ) {
            // 切换页码重新计算oldVal
    ...
    handleSelectionChange(val) {
      ...
      if (valKeys.length < oldValKeys.length) {
        ...
        this.selectedTableData = this.selectedTableData.filter(
          item => !delKey.includes(item[this.selectFieldUnique])
        );
        this.$emit("selection-change", this.selectedTableData);
      } else {
        const addKey = valKeys.filter(item => !oldValKeys.includes(item));
        const addVal = val.filter(item =>
          addKey.includes(item[this.selectFieldUnique])
        );
