...
class Data
{
     ...
     * @return array
     */
    public static function getArrSupIds($list, array $ids = [], $key = 'id', $pkey = 'pid', array $sup = [])
    {
        if (empty($ids)) return [];
        $list = array_combine(array_column($list, $key), array_column($list, $pkey));
        foreach ($ids as $vo) self::joinPid($list, $vo, $sup);
